...
//     // This code block creates a `test.log` file with one full record and one fragmented record.
//     {
//         use std::io::Write;
//         let mut writer = File::create("test.log").unwrap();
//         // Record 1: FULL, "hello"
//         let data1 = b"hello";
//         let type1 = FULL_TYPE;
//         let crc1 = crc32c::crc32c(&[&[type1], data1].concat());
//         let masked_crc1 = (((crc1 >> 15) | (crc1 << 17)).wrapping_add(0xa282ead8));
//         writer.write_all(&masked_crc1.to_le_bytes()).unwrap();
//         writer.write_all(&(data1.len() as u16).to_le_bytes()).unwrap();
//         writer.write_all(&[type1]).unwrap();
//         writer.write_all(data1).unwrap();

//         // Record 2: FRAGMENTED, "world-leader"
//         let data2_first = b"world-";
//         let type2_first = FIRST_TYPE;
//         let crc2_first = crc32c::crc32c(&[&[type2_first], data2_first].concat());
//         let masked_crc2_first = (((crc2_first >> 15) | (crc2_first << 17)).wrapping_add(0xa282ead8));
//         writer.write_all(&masked_crc2_first.to_le_bytes()).unwrap();
//         writer.write_all(&(data2_first.len() as u16).to_le_bytes()).unwrap();
//         writer.write_all(&[type2_first]).unwrap();
//         writer.write_all(data2_first).unwrap();

//         let data2_last = b"leader";
//         let type2_last = LAST_TYPE;
//         let crc2_last = crc32c::crc32c(&[&[type2_last], data2_last].concat());
//         let masked_crc2_last = (((crc2_last >> 15) | (crc2_last << 17)).wrapping_add(0xa282ead8));
//         writer.write_all(&masked_crc2_last.to_le_bytes()).unwrap();
//         writer.write_all(&(data2_last.len() as u16).to_le_bytes()).unwrap();
//         writer.write_all(&[type2_last]).unwrap();
//         writer.write_all(data2_last).unwrap();
//     }

//     // Now, use the LogReader
//     println!("Reading records from 'test.log':");
//     match LogReader::new("test.log") {
//         Ok(reader) => {
//             for (i, record_result) in reader.enumerate() {
//                 match record_result {
//                     Ok(record) => {
//                         println!("Record {}: {:?} ({} bytes)", i + 1, String::from_utf8_lossy(&record), record.len());
//                     }
//                     Err(e) => {
...
// Key Differences and Advantages of the Rust Version:

// Type Safety: Rust guarantees at compile time that types are correct, eliminating an entire class of bugs common in dynamic languages.

// Explicit Error Handling: The use of Result and Option forces you to handle all possible failure states, making the code more robust. The program will not simply "crash" with an unhandled exception.

// Performance: The Rust version will be significantly faster and consume less memory, as it is compiled to native machine code and avoids the overhead of an interpreter and garbage collector.

// Idiomatic: Implementing the Iterator trait is the standard way to create sequences in Rust. This means you can use powerful methods like .map(), .filter(), .collect() directly on your LogReader, for free.

// Checksum: I've included the correct "masking" logic for CRC32C that LevelDB uses, which was commented out in your Python code but is fundamental for data validation. You can comment out the Err line if you don't want the program to stop due to a checksum failure.
